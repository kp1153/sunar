...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/session-check").then((r) => r.json()).then((data) => {
  ...
  return (
    <div className="min-h-screen bg-[#F9F9F9] pb-24">
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        {userInfo && (
          <div className="bg-white rounded-2xl p-5 border border-zinc-100 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-zinc-800">{userInfo.name || "—"}</p>
                <p className="text-zinc-400 text-sm">{userInfo.email}</p>
                <p className="text-xs mt-0.5">
                  <span className={`font-black ${userInfo.status === "active" ? "text-green-500" : "text-amber-500"}`}>
        ...
        <div className="bg-white rounded-2xl p-5 border border-zinc-100 shadow-sm space-y-4">
          <div>
            ...
            <p className="text-zinc-400 text-xs mt-1">यह जानकारी बिल पर छपेगी</p>
          </div>

          {[
            { key: "shopName", label: "दुकान का नाम", placeholder: "जैसे: श्री राम ज्वेलर्स" },
            { key: "ownerName", label: "मालिक का नाम", placeholder: "जैसे: रामप्रसाद जी" },
            { key: "phone", label: "मोबाइल नंबर", placeholder: "जैसे: 9876543210", type: "tel" },
            { key: "gstin", label: "GSTIN (वैकल्पिक)", placeholder: "जैसे: 09AAAAA0000A1Z5" },
          ...
          <div>
            <label className="block text-xs font-black text-zinc-400 uppercase tracking-wider mb-2">पता</label>
            <textarea
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-zinc-200 flex justify-around items-center py-2">
        ...
        <Link href="/dashboard/settings" className="flex flex-col items-center gap-0.5 px-3 py-1 text-[#AA7D6E]"><span className="text-xl">⚙️</span><span className="text-[10px] font-bold">सेटिंग</span></Link>
      </nav>
    </div>
  );
}