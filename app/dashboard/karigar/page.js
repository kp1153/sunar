...
export default function KarigarPage() {
  ...
  const addKarigar = async () => {
    if (!karigarForm.name.trim()) {
      setError("कारीगर का नाम डालें");
      return;
    }
    setSaving(true);
    try {
  ...
  const addWork = async () => {
    if (!workForm.karigarId) {
      setError("कारीगर चुनें");
      return;
    }
    if (!workForm.description.trim()) {
      setError("काम का विवरण डालें");
      return;
    }
    if (!workForm.issuedWeight || parseFloat(workForm.issuedWeight) <= 0) {
      setError("वजन डालें");
      return;
    }
    setSaving(true);
  ...
  const returnWork = async () => {
    if (
      !returnForm.returnedWeight ||
      parseFloat(returnForm.returnedWeight) <= 0
    ) {
  ...
  const pending = works.filter(
    (w) =>
      w.status === "pending" &&
      (activeKarigar ? w.karigarId === activeKarigar : true),
  );
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-6 space-y-4">
            ...
            <input
              ...
              onChange={(e) =>
                setKarigarForm({ ...karigarForm, name: e.target.value })
              }
            ...
            <input
              ...
              onChange={(e) =>
                setKarigarForm({ ...karigarForm, phone: e.target.value })
              }
            ...
            <div className="flex gap-3">
              <button
                onClick={addKarigar}
                disabled={saving}
                className="flex-1 bg-[#AA7D6E] text-white font-black py-4 rounded-2xl disabled:opacity-50"
              >
              ...
              <>
                <div className="flex flex-wrap gap-2">
                  {karigars.map((k) => (
                    <button
                      key={k.id}
                      onClick={() =>
                        setWorkForm({ ...workForm, karigarId: k.id.toString() })
                      }
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex gap-2">
                    {["सोना", "चाँदी"].map((m) => (
                      <button
                        key={m}
                        onClick={() =>
                          setWorkForm({ ...workForm, metalType: m })
                        }
                        className={`flex-1 py-3 rounded-xl font-bold text-sm ${
                          workForm.metalType === m
                            ? "bg-[#AA7D6E] text-white"
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setWorkForm({ ...workForm, issuedWeight: e.target.value })
                    }
                    className="w-full p-4 bg-zinc-50 rounded-2xl ring-1 ring-zinc-200 outline-none text-base"
                    type="number"
                    step="0.01"
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-6 space-y-4">
            ...
            <div className="bg-zinc-50 rounded-2xl p-4">
              ...
              <p className="text-sm mt-2">
                ...
                <span className="font-black text-zinc-800">
                  {selected.issuedWeight}g {selected.metalType}
                </span>
              </p>
            </div>
              ...
              parseFloat(returnForm.returnedWeight) > 0 && (
                <div
                  className={`rounded-2xl p-4 ${
                    selected.issuedWeight -
                      parseFloat(returnForm.returnedWeight) >
                    0
            ...
            <input
              ...
              onChange={(e) =>
                setReturnForm({ ...returnForm, labourCharge: e.target.value })
              }
              className="w-full p-4 bg-zinc-50 rounded-2xl ring-1 ring-zinc-200 outline-none text-base"
              type="number"
            />
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-orange-50 border border-orange-100 rounded-2xl p-4">
            ...
            <p className="text-xl font-black text-zinc-800 mt-1">
              {totalIssuedPending.toFixed(2)}g
            </p>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            ...
            {karigars.map((k) => (
              <button
                key={k.id}
                onClick={() =>
                  setActiveKarigar(activeKarigar === k.id ? null : k.id)
                }
                className={`px-4 py-2 rounded-xl font-bold text-sm flex-shrink-0 transition ${
                  activeKarigar === k.id
                    ? "bg-[#AA7D6E] text-white"
        ...
        {loading && (
          <div className="space-y-3">
            {[1, 2].map((i) => (
              <div
                key={i}
                className="bg-white rounded-2xl p-4 border border-zinc-100 animate-pulse h-28"
              />
            ))}
          </div>
        )}
        ...
        {!loading && done.length > 0 && (
          <>
            ...
            {done.map((w) => (
              <div
                key={w.id}
                className="bg-zinc-50 rounded-2xl p-4 border border-zinc-100 opacity-75"
              >
